...
$url = "https://$api_key:$token@blinginglight.myshopify.com/admin/api/2021-07/orders.json";
$curl = curl_init();
          ...
          foreach($item['line_items'] as $item_list){
			...
			$quantity = $item_list['quantity'];   
			 $price = $item_list['price_set']['presentment_money']['amount'];
					 ...
					 $sku = $item_list['sku'];
			$arr = array('line_id'=>$id,'firstname'=>$first_name, 'lastname'=>$last_name, 'phone'=>$phone, 'email'=>$email, 'order_id'=>$order_id, 'quantity'=>$quantity, 'price'=>$price, 'productname'=>$product_name, 'sku'=>$sku, 'product_id'=>$product_id, 'address'=>$address1, 'city'=>$city, 'province'=>$province );
...
if(empty($fetch['line_id']) == !empty($total[$i]['line_id'])){
 $sql = "INSERT INTO rate(firstname, lastname, phone, email, orderid, quantity, amount, product, sku, line_id, product_id, address, city, state) values( '".$total[$i]['firstname']."', '".$total[$i]['lastname']."', '".$total[$i]['phone']."', '".$total[$i]['email']."', '".$total[$i]['order_id']."', '".$total[$i]['quantity']."', '".$total[$i]['price']."', '".$total[$i]['productname']."', '".$total[$i]['sku']."', '".$total[$i]['line_id']."', '".$total[$i]['product_id']."', '".$total[$i]['address']."', '".$total[$i]['city']."', '".$total[$i]['state']."')";
 $query = mysqli_query($conn, $sql);
}
